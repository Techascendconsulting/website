...
const Step: React.FC<{ k: string; title: string; body: string }> = ({ k, title, body }) => (
  <div className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
    <div className="text-lime-600 font-mono text-sm tracking-widest">{k}</div>
    ...
    <main className="pt-28 bg-white min-h-screen">
      ...
      <section ref={hero.ref as any} className="pb-10">
        <div className="max-w-7xl mx-auto px-6 grid grid-cols-1 lg:grid-cols-2 gap-8 items-center">
          <div style={{opacity: hero.visible ? 1 : 0, transform: hero.visible ? 'none' : 'translateY(10px)', transition: 'all 380ms ease-out'}}>
            <h1 className="text-4xl md:text-5xl font-bold text-slate-900">Training Pathway: Learn → Do → Place</h1>
            <p className="mt-4 text-slate-600 text-lg">A clear journey that builds capability through real work—so you join client projects ready to deliver.</p>
          ...
          <div>
            <img src="https://images.pexels.com/photos/3184291/pexels-photo-3184291.jpeg" alt="Learning cohort" className="w-full h-[360px] object-cover rounded-2xl border border-slate-200 shadow-2xl" loading="lazy" />
          </div>
        </div>
      </section>
      ...
      <section ref={gallery.ref as any} className="py-10">
        <div className="max-w-7xl mx-auto px-6">
          <h2 className="text-2xl font-semibold text-slate-900">What you’ll deliver</h2>
          <div className="mt-6 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
            ...
            {t:'Test plans', img:'https://images.pexels.com/photos/3861969/pexels-photo-3861969.jpeg'},{t:'Backlogs', img:'https://images.pexels.com/photos/3861944/pexels-photo-3861944.jpeg'},{t:'Stakeholder comms', img:'https://images.pexels.com/photos/5151697/pexels-photo-5151697.jpeg'}].map((g, i) => (
              <div key={g.t} className="rounded-2xl overflow-hidden border border-slate-200 bg-white shadow-sm">
                ...
                <div className="p-4">
                  <div className="text-slate-900 font-medium">{g.t}</div>
                </div>
              </div>
            ))}
          </div>
